...
function isRequiredFilterHeader(title: string): boolean {
    return (
        matchesHeader(title, LEVEL1_ALIASES) ||
        matchesHeader(title, LEVEL2_ALIASES)
    );
}
...
function convertWindowsPathToWslMount(pathLike: string): string | null {
    const normalizedWindowsPath = normalizeWindowsDrivePath(pathLike);
    if (!normalizedWindowsPath) {
        return null;
    }

    const driveMatch = /^([a-zA-Z]):\\(.+)$/.exec(normalizedWindowsPath);
    if (!driveMatch) {
        return null;
    }
...
function normalizeCrossPlatformAbsolutePath(pathLike: string): string | null {
    const normalizedWindowsPath = normalizeWindowsDrivePath(pathLike);
    if (!normalizedWindowsPath) {
        return null;
    }
    ...
    if (process.platform === "linux") {
        const wslMountedPath = convertWindowsPathToWslMount(
            normalizedWindowsPath,
        );
        if (wslMountedPath) {
            return wslMountedPath;
        }
    }
        ...
        envRootsRaw
            ...
            .forEach((item) => {
                const normalizedAbsolutePath =
                    normalizeCrossPlatformAbsolutePath(item);
                if (normalizedAbsolutePath) {
                    roots.push(normalizedAbsolutePath);
                } else {
...
function locateImageRootDirectory(rootDirName: string): string | null {
    ...
    for (const searchRoot of searchRoots) {
        ...
        if (found) {
            // eslint-disable-next-line no-console
            console.log(
                `[ImageRootSearch] Found "${rootDirName}" at: ${found}`,
            );
...
function normalizeHyperlinkToAbsolutePath(
    ...
    if (/^file:\/\//i.test(trimmed)) {
        try {
            ...
            const normalizedResolved =
                normalizeCrossPlatformAbsolutePath(resolved) ?? resolved;
            hyperlinkPathCache.set(trimmed, normalizedResolved);
    ...
    const normalizedAbsolutePath = normalizeCrossPlatformAbsolutePath(trimmed);
    if (normalizedAbsolutePath) {
        hyperlinkPathCache.set(trimmed, normalizedAbsolutePath);
